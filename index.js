...
Vue.component("card", {
  ...
  created: function () {
    ...
    const countup = () => {
      ...
      this.liked = Math.min(lim, this.liked + Math.max(1, Math.ceil(Math.pow(sum / 1000 + randInt(0, 5), exp1))));
      this.retweeted = Math.min(lim, this.liked + Math.max(1, Math.ceil(Math.pow(sum / 1000 + randInt(0, 5), exp2))));
      tm = 1000 + randInt(1000, 2000);
      sum += tm;
      if (this.liked != lim || this.retweeted != lim) {
        setTimeout(countup, tm / 25);
      }
    }
